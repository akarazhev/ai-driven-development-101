...
Set up the foundation of your Confluence automation app. This milestone focuses on project structure, development
...
Task: Design project structure for a full-stack Confluence automation app
Constraints:
...
project/
  backend/
    ...
    providers/      # Social attachment adapters
...
Task: Design database schema for Confluence automation app
Constraints:
- Tables: AttachmentAsset, Page, PageAttachment, Schedule, PublishLog
...
- `attachment_assets`: id, filename, content_type, size, storage_path, alt_text
- `pages`: id, text, created_at, updated_at, author_id
- `page_attachment`: id, page_id, attachment_id, position
- `schedules`: id, page_id, scheduled_at, status, attempt_count, last_error
- `publish_logs`: id, page_id, provider, external_id, status, message, created_at
...
```text
BaseProvider:
  publish(text: string, attachment_paths: string[]) -> (external_id: string, message: string)
...
- Methods for: health check, attachment, pages, schedules
- Error handling
- TypeScript types
...
Once M1 is complete, proceed to **[M2: Core Pageing Flows](./06.2-milestone-2-core-flows.md)**
