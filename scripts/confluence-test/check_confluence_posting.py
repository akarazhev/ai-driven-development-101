...
def get_credentials():
    """
    Get credentials from .env file, environment variables, or prompt user for input.
    
    Returns:
        Tuple of (base_url, username, api_token, space_key)
    """
    import os
    import getpass
    ...
    project_root = script_dir.parent.parent  # Go up from scripts/confluence-test to project root
    
    env_files = [
        script_dir / '.env',           # .env in script directory
    ...
    # Support both CONFLUENCE_SPACE_KEY and CONFLUENCE_DEFAULT_SPACE
    ...
    if not base_url:
        base_url = input("Enter Confluence base URL (e.g., https://your-domain.atlassian.net/confluence): ").strip()
        if not base_url:
            print("Error: Base URL is required")
            exit(1)
    
    if not username:
        username = input("Enter Confluence username: ").strip()
        if not username:
            print("Error: Username is required")
            exit(1)
    
    if not api_token:
        api_token = getpass.getpass("Enter Confluence API token: ").strip()
        if not api_token:
            print("Error: API token is required")
            exit(1)
    
    if not space_key:
        space_key = input("Enter Confluence space key (e.g., SPGAC): ").strip()
        if not space_key:
            print("Error: Space key is required")
            exit(1)
    
    return base_url, username, api_token, space_key


def main():
    """Main function to run the test."""
    # Get credentials from environment or user input
    base_url, username, api_token, space_key = get_credentials()
